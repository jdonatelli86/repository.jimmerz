...
#############Defined Strings#############
addon_id     = 'plugin.video.jimmerz-tv'
...
Guide = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.jimmerz-tv/resources/catchup', 'guide.xml'))
GuideLoc = xbmc.translatePath(os.path.join('special://home/addons/plugin.video.jimmerz-tv/resources/catchup', 'g'))
...
def start():
	if username=="":
		...
		if auth == "":
			...
			line2 = "Welcome to jimmerz-tv" 
			line3 = ('[COLOR ghostwhite]%s[/COLOR]'%user)
			xbmcgui.Dialog().ok('jimmerz-tv', line1, line2, line3)
			tvguidesetup()
			addonsettings('ADS2','')
			xbmc.executebuiltin('Container.Refresh')
			home()
...
##########################################
def catchup():
    loginurl   = "http://nosleepco.net:8080/get.php?username=" + username + "&password=" + password + "&type=m3u_plus&output=ts"
    try:
...
def _pbhook(numblocks, blocksize, filesize, dp, start_time):
        ...
        if dp.iscanceled():
            ...
            dialog.ok("jimmerz-tv", 'The download was cancelled.')
...
def searchdialog():
	search = control.inputDialog(heading='Search jimmerz-tv:')
	if search=="":
		return
	else:
		return search
...
def addonsettings(url,description):
	if   url =="CC":
		...
		if dialog==0:
			...
			xbmcgui.Dialog().ok('jimmerz-tv', 'Set Advanced Settings')
		elif dialog==1:
			...
			xbmcgui.Dialog().ok('jimmerz-tv', 'Advanced Settings Removed')
	elif url =="ADS2":
		...
		if dialog==0:
			ivueint()
			xbmcgui.Dialog().ok('jimmerz-tv', 'iVue Integration Complete')
		elif dialog==1:
			pvrsetup()
			xbmcgui.Dialog().ok('jimmerz-tv', 'PVR Integration Complete')
		elif dialog==2:
			pvrsetup()
			ivueint()
			xbmcgui.Dialog().ok('jimmerz-tv', 'PVR & iVue Integration Complete')
	elif url =="ST":
		xbmc.executebuiltin('Runscript("special://home/addons/plugin.video.jimmerz-tv/resources/modules/speedtest.py")')
	elif url =="META":
...
def asettings():
	choice = xbmcgui.Dialog().yesno('jimmerz-tv', 'Please Select The RAM Size of Your Device', yeslabel='Less than 1GB RAM', nolabel='More than 1GB RAM')
	if choice:
		lessthan()
	else:
		morethan()
...
def correctPVR():
	...
	addon = xbmcaddon.Addon('plugin.video.jimmerz-tv')
	...
	loginurl   = "http://nosleepco.net:8080/get.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
	EPGurl     = "http://nosleepco.net:8080/xmltv.php?username=" + username_text + "&password=" + password_text + "&type=m3u_plus&output=ts"
...
def tvguidesetup():
		dialog = xbmcgui.Dialog().yesno('jimmerz-tv','Would You like us to Setup the PVR Guide for You?')
		if dialog:
