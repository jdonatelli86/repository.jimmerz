...
def index():
	...
	#addDir ('Hot Fixes'        ,'viewpack',   icon=ICONMAINT,   themeit=THEME1)
...
def tweaks():
	...
	addFile('Include Video Cache in Clear Cache: %s' % includevid.replace('true',on).replace('false',off), 'togglecache', 'includevideo', icon=ICONMAINT, themeit=THEME1)
	if includevid == 'true':
		...
		addFile('--- Include Zen: %s' % includezen.replace('true',on).replace('false',off), 'togglecache', 'includezen', icon=ICONMAINT, themeit=THEME3)
